...
class GrassHopper(Herbivor):
    ...
    def __init__(self, species_id: int, alive_status: bool, procreate_cool_down: float, organism_position: List[float], 
                 animal_id: int, hunger: float, max_hunger: float, thirst: float, max_thirst: float, speed: float, mass: float, 
                 exhaustion: float, max_exhaustion: float, visibility_range: float, detection_range: float, feeding_range: float, 
                 sleep_duration: int, detection_multiplier: float, warned: bool, in_danger: bool, ready_to_mate: bool, 
                 consumable_organisms: List[int], consumable_resources: List[int], all_known_resources: List[Any], decision_duration: int):
